...
class MyWF(hk.Module):
    def __init__(
        ...
        self.env =
ExponentialEnvelopes(hamil,n_determinants,isotropic=False,
per_shell=False, per_orbital_exponent=False,
spin_restricted=False, init_to_ones=False, softplus_zeta=False)
        self.cusp_electrons =
ElectronicCuspAsymptotic(same_scale=0.25,anti_scale=0.5,alpha=1,
trainable_alpha=True, cusp_function=PsiformerCusp())
    ...
    def __call__(self, phys_conf:PhysicalConfiguration, _):
        ...
        elec_nuc_diffs = pairwise_diffs(phys_conf.r,
phys_conf.R).reshape(n_elec, -1)
        elec_nuc_dist, elec_nuc_diffs = elec_nuc_diffs[-1:]**0.5,
elec_nuc_diffs[:3]
        elec_elec_diffs = pairwise_diffs(phys_conf.r, phys_conf.r)
        elec_elec_dist = elec_elec_diffs[-1:]**0.5

        # INPUT FEATURES
        # ___________________________
        rescaled_diffs =
elec_nuc_diffs*jnp.log(1+elec_nuc_dist)/elec_nuc_dist
        elec_emebeddings = jnp.concatenate((rescaled_diffs,
jnp.log(1+elec_nuc_dist), jnp.concatenate((jnp.ones(self.n_up),
-jnp.ones(self.n_down)))[...,None]),axis=-1)
        # ___________________________
        ...
        for _ in range(3):
            h = h + jnp.einsum('ijk->ik',hk.nets.MLP([32,64],
activation=jax.nn.sigmoid, with_bias=False)(elec_elec_diffs))
        f =
hk.Linear(self.n_determinants*n_elec)(h).reshape(n_elec,self.n_determinants,n_elec).swapaxes(0,1)
        # ___________________________
        ...
        xs_shift = jnp.where(~jnp.isinf(xs_shift), xs_shift,
