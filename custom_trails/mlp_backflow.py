...
class MyWF(hk.Module):
    def __init__(
        ...
        self.env = ExponentialEnvelopes(hamil,n_determinants,isotropic=False, per_shell=False, per_orbital_exponent=False, spin_restricted=False, init_to_ones=False, softplus_zeta=False)
        self.cusp_electrons = ElectronicCuspAsymptotic(same_scale=0.25,anti_scale=0.5,alpha=1, trainable_alpha=True, cusp_function=PsiformerCusp())
    @property
    ...
    def __call__(self, phys_conf:PhysicalConfiguration, _):
        ...
        elec_nuc_diffs = pairwise_diffs(phys_conf.r, phys_conf.R).reshape(n_elec, -1) elec_nuc_dist, elec_nuc_diffs = elec_nuc_diffs[-1:]**0.5, elec_nuc_diffs[:3]
        elec_elec_diffs = pairwise_diffs(phys_conf.r, phys_conf.r)
        elec_elec_dist = elec_elec_diffs[-1:]**0.5

        # INPUT FEATURES
        # ___________________________
        rescaled_diffs = elec_nuc_diffs*jnp.log(1+elec_nuc_dist)/elec_nuc_dist
        elec_emebeddings = jnp.concatenate((rescaled_diffs, -jnp.ones(self.n_down)))[...,None]),axis=-1)
        # ___________________________
        ...
        for _ in range(3):
            h = h + jnp.einsum('ijk->ik',hk.nets.MLP([32,64], activation=jax.nn.sigmoid, with_bias=False)(elec_elec_diffs))
        ...
        same_dists = jnp.concatenate(
            [triu_flat(elec_elec_dist[idxs, idxs]) for idxs in self.spin_slices], axis=-1,
        )
